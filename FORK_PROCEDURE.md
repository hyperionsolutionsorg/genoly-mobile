...
**Last verified:** 2026-05-05 (design phase — not yet executed).
...
- [ ] The ESLint forkability boundary rule is active and passes: `cd genoly-family-web && npm run lint` against `convex/fitness/` shows no `import/no-restricted-paths` violations. (If it fails, fix the violations before forking — they indicate Genoly→fitness or fitness→non-allow-list imports that will break extraction.)
- [ ] The allow-list of Genoly files that fitness imports (defined in `eslint.config.js` zone 1 `except` array) matches the list in this document's §"Shared code that gets copied" section. If the ESLint allow-list has grown, update this doc first.
...
These are the files that the ESLint rule (`eslint.config.js` zone 1 `except` array) explicitly permits fitness to import. **If this list has grown** since 2026-05-05 (check the eslint config), update both the eslint config and this document together.
...
- [ ] **Disable fitness in Genoly.** In `genoly-family-web`:
  ...
  - Run `npm run lint` — should still pass; the eslint forkability rule remains harmless on an empty allow-list reference.
