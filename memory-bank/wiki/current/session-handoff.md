...
updated: 2026-07-13b (HEALTH READ PIPELINE ROOT-CAUSED + FIXED — PR #53 merged, main `d7c3eba`. Two structural bugs since Step 4/12/5: (1) adapters gated reads on a per-instance grant flag no reader ever set (factory returns fresh instances) → every read silently `[]`; (2) the fitness pipeline had NO producer — `SyncQueue.enqueue()` had zero call sites, so `sync/daily` never got device data. Fixed: lazy per-process init + `getGrantedPermissions()` on the read path; NEW `utils/healthSync.ts` producer (30-day initial/7-day steady) wired into dashboard refresh + background task + permissions screen; silent catches now surface reasons. Local APK v6 delivered `~/Desktop/genoly-local-convex.apk`; emulator-verified through "Synced just now" (Android-16 image HAS Health Connect; store empty → real-data proof = operator's Samsung sideload). Jest 407/34, tsc clean. Prior: 2026-07-13 Rule #0 catch-up (main was `93181cd`; #46/#49/#50/#51/#52 all merged). See log.md [2026-07-13] merge entry.)
status: active
---
...
## What's next (pending — consolidated 2026-07-13b)

1. **Samsung sideload verification (operator, in progress)** — install the fresh `~/Desktop/genoly-local-convex.apk` (v6, carries #53) on the physical Samsung (same Wi-Fi as the Mac, local Convex up): sign in → Grant access (real HC permission sheet) → Activity should show REAL steps/calories/distance + "Synced just now"; a joined challenge's "Sync now" should toast a step count. Failures now surface reasons (no-permission / disabled / store-unavailable) instead of silence. **iOS not rebuilt** — the #53 adapter fix applies to HealthKitAdapter too but is untested on a device.
