...
updated: 2026-07-13j (+#63 origin attribution r63 — r62 field result CONFIRMED the tier fix (calories/distance now aggregate-duration; their no-data is true source-side absence); diag line now names contributing packages. APK v15 delivered. Prior 2026-07-13i (+#62 aggregate tier chain — r61 diag caught upstream rn-health-connect bug: period requests use Instant filter for Calories/Distance (SDK rejects); now period→duration→raw per metric, rev r62. Steps CONFIRMED correct (= HC dedup total; delta vs Samsung dashboard is Samsung export behavior). APK v14 delivered. Prior 2026-07-13h (+#61 on-device read diagnostics — Settings shows rev/read-path/per-metric outcome so field screenshots replace adb. APK v13 delivered; awaiting operator r61 diagnostic screenshot. Prior 2026-07-13g (+#60 aggregate INSTANT contract — #59 naive-local strings threw in native Instant.parse, silent fallback overcounted 3257-vs-2735; now local-midnight instants → true HC daily aggregates. APK v12 delivered. Prior 2026-07-13f (+#59 step-accuracy fix — UTC-window bug dropped evening records west of UTC + raw-record summing replaced by HC aggregateGroupByPeriod (dedup). APK v11 delivered. Prior 2026-07-13e (+#57 30-day history sync, web #277 1-yr retention + DELETE /api/fitness/health-data (deployed local), #58 mobile controls (turn off sync / delete my data / queue clearAll). APK v10 delivered. FIELD-VERIFIED: real steps (145) flowed Samsung→HC→Genoly→Convex on the operator Samsung. Prior 2026-07-13d (+#56 Android-14+ HC client declaration — the REAL Samsung grant blocker: missing VIEW_PERMISSION_USAGE/HEALTH_PERMISSIONS activity-alias meant HC never recognized Genoly as a client, so no permission sheet and no manual grant path. Plugin adds the alias; APK v8 delivered, main `b06b34d`. See log.md. Prior 2026-07-13c (+#54 HC grant recovery (rate-limited permission contract → pre/post getGrantedPermissions checks + openHealthSettings manual path) and #55 FULL GAMES PORT (hub /games + all 8 web games native, cross-platform-identical daily seeds, dailies record recordDailyCompletion; deps d3-geo/topojson-client/world-atlas). main `9a27a91`; Jest 419/34; tsc clean; APK v7 delivered + emulator-verified. Prior 2026-07-13b: #53 health read pipeline fix)
status: active
---

# Progress

> **2026-07-13c — #54 + #55 merged (main `9a27a91`), local APK v7 delivered.** #54: Samsung's "No access granted" root-caused to Health Connect's permission-contract rate limit (auto-resolves empty after ~2 denials, attempts burned across earlier APK rounds); adapters now honor OS-persisted grants around the contract and expose `openHealthSettings()`; the permissions screen guides a manual grant + retry. #55 (operator request): games hub + all 8 games ported native (Wordle, Word Search, Atlas Quiz on d3-geo/react-native-svg, Generation Sprint, Who Am I?, This or That, Family Connections, Timeline Tap); registry rewritten to the web's flat model; shared daily-seed libs verbatim so web + mobile play identical dailies; Connections/Timeline Tap record `gameCompletions:recordDailyCompletion`. Jest **419/34**, tsc clean; emulator-verified (Games section, hub lock states from live getGamesContext, Wordle round-trip). Awaiting operator's Samsung check of health recovery + games on v7. Details: `log.md` [2026-07-13] merge entries.
