...
updated: 2026-07-13 (Rule #0 catch-up — reconciled to real merge history: main `93181cd`, no open mobile PRs. ✅ MERGED: Phase-1/tree run #27–#40 (§15 Phase 1 COMPLETE, ApiClient 16/20); security #46 (fitness HTTPS transport) + #49 (app.config.ts prod-URL fail-closed); fixes #50 (HC delegate) / #51 (ExploreCanvas raster cap, ±4 crash) / #52 (expo-secure-store — bearer token persists, health syncs). Pending: mobile EAS prod env vars; local-APK stash reconcile; physical-Samsung + iOS verify; token-recovery follow-up. See log.md [2026-07-13]. Prior: 2026-07-11b (prod-URL #49 + transport #46, then OPEN). Prior: 2026-07-09 (Phase-1/tree run). Prior: 2026-06-11)
status: active
---
