...
## [2026-07-09] note | Local Android emulator build works — toolchain + foojay workaround + 2 fixes (PR #36)

First successful LOCAL Android build/run on the restored Mac (emulator visual pass, task #300). Toolchain now in place: Android Studio + SDK **API 36** google_apis arm64 system image, AVD **geno_test** (Pixel 7), JDK 17 via `brew install openjdk@17` + Android Studio's bundled JBR 21, cmdline-tools installed into `~/Library/Android/sdk` (the brew cmdline-tools were too old to read the SDK's v4 package XML — use the SDK's own `cmdline-tools/latest`).

**Load-bearing gotcha:** RN 0.85.3 pins **foojay-resolver 0.5.0** (references `JvmVendorSpec.IBM_SEMERU`, removed in Gradle 9) against its own **Gradle 9.3.1** wrapper, so JDK-toolchain auto-download crashes at `DistributionsKt.<clinit>`. Fix WITHOUT touching node_modules: in `~/.gradle/gradle.properties` (machine-local, not in repo) set `org.gradle.java.installations.auto-download=false` + `org.gradle.java.installations.paths=<openjdk17>,<JBR21>` so Gradle finds the JDK-17 toolchain locally and never invokes foojay. EAS cloud builds are unaffected (they pin their own toolchain).

**Two app bugs found + fixed (PR #36, unmerged):** (1) minSdk 24→26 via expo-build-properties — Health Connect (`androidx.health.connect` via react-native-health-connect) requires 26; debug build failed at manifest merge. (2) `useHasProTenantAccess()` fired `useQuery(listMyTenants)` unconditionally → signed-out cold start threw "Not authenticated" render error before AuthGate could route to login; now skips the query while unauthenticated. Device-verified: login + signup render clean (partial screenshot sweep in `vault/mobile-screenshots/`; authenticated surfaces still need a dev sign-in). Suite 385/34; tsc clean.

## [2026-07-09] merge | Emulator-run fixes (#36) + Genoly app icon (#37) merged

#36 `94b4fa2` — minSdk 24→26 (expo-build-properties; Health Connect requires it) + guard useHasProTenantAccess's listMyTenants query while unauthenticated (signed-out cold start was throwing "Not authenticated" render error before AuthGate routed to login). Device-verified: login/signup now render clean.
...
## [2026-07-10] merge | iOS walkthrough fixes (#40) — HealthKit new-arch proxy + real nav titles

Operator's authenticated iOS walkthrough surfaced: (1) Grant access → "undefined is not a function" — RN 0.85 bridgeless NativeModules interop exposes methods as lazy NON-ENUMERABLE proxy props, so react-native-health's `Object.assign({}, AppleHealthKit, {Constants})` ships Constants-only. Diagnosed with on-device probes (Object.keys empty; direct access = function). Fix: HealthKitAdapter.loadNativeModule returns a delegating Proxy over NativeModules.AppleHealthKit when the JS export is stripped. (2) Native headers showed raw route names ("(auth)/login", back-label "(tabs)") — root Stack declared only 2 screens. Now full-screen surfaces hide native headers; pushed screens get real titles + chevron-only back; tabs no longer double-title; the "(gated)" WARN fixed ((gated)→(gated)/paywall). tsc clean; 372/34.

Operator Qs answered: email-keyboard oddity = Gboard floating mode (device-level, app config correct); Explore pan/pinch code verified correct (RNGH pan+pinch simultaneous on ZoomPanView) — iOS Simulator input mechanics: click-drag pans, mouse WHEEL does nothing on iOS sim (unlike Android emulator), pinch = Option+click-drag; page scrolling = same click-drag. Gotcha banked: `CI=1 expo run/start` puts Metro in CI mode (NO file watcher — edits silently never reach the app); use plain `expo start --dev-client` for iterative work. Clean ios rebuild reinstalls the app → secure-store session wiped (operator must re-sign-in).
...
## [2026-07-10] merge | Fullscreen landscape Explore (#43, supersedes #42 in-place rotate)

Operator screenshot showed #42's in-place rotation kept all chrome (tab bar, brand bar, picker, title) → broken, space wasted. #43: Explore expand button opens a RN <Modal> covering everything; canvas gets the whole screen; locks landscape on open / portrait on close (programmatic → auto-rotate-off safe); shares graph/anchor/radius state so pan/pinch/zoom + re-anchor work and persist; SafeAreaView all-edges for the notch; floating ✕ to exit. JS-only (expo-screen-orientation from #42 already native; safe-area-context linked) → fast-refresh, no rebuild. tsc clean; 372/34. Fullscreen visual behind login — operator to confirm post-sign-in.

## [2026-07-10] merge | Fullscreen exit fix + ceiling ⓘ (#44) — first adb-driven on-device e2e

Operator hit the iOS "Modal presented with 0x18 orientations mask / app only supports 0x6" on fullscreen exit (crashes in prod) + asked to reclaim the ceiling-banner real estate. #44: (a) prebuild regenerated native projects so app.json orientation=default actually lands (Info.plist all 4 orientations; Android manifest unspecified) — GOTCHA: `expo run` does NOT re-run prebuild when native dirs exist, so app.json config changes silently don't apply until `expo prebuild`; (b) Modal supportedOrientations gains 'portrait' (dismissal transition runs during the portrait relock); (c) "More generations exist" Banner strip → ⓘ button beside the Generations chips → toast (no-silent-caps parity, zero canvas cost).

VERIFICATION FIRST: full adb-driven e2e on the Android emulator — signed in as e2e TEST_ADMIN (dev deployment; Pro tenant passes paywall; NOTE: mobile login's client-side reserved-TLD email check blocks @genoly.test creds — temporarily bypassed in authSchemas.ts during the test, REVERTED after; consider a DEV-build exemption if mobile e2e becomes routine). Verified: expand→fullscreen landscape (ROTATION_90, no chrome), pan swipe + ⓘ toast inside fullscreen, exit via ✕ AND system back → portrait restored, inline intact; ZERO logcat errors across repeated cycles. iOS Info.plist verified post-prebuild; operator to re-test iOS exit. Android emulator left signed in as test-admin@genoly.test.
...
## [2026-07-11] note | Transport hardening — fitness client HTTPS guard + Android cleartext off (PR #46 OPEN)

Part of the cross-repo security run (web `_security-encryption-and-cross-tree-access-brief.md`; web PRs #271/#272/#273). Mobile slice = Requirement 1 transport, branch `security/mobile-transport-hardening`, **PR #46 OPEN pending operator merge.**

**What shipped:** (1) `packages/api-client/src/client.ts` — new `assertSecureBaseUrl()` called in the `FetchApiClient` constructor: refuses cleartext `http://` to any NON-local host (loopback/`localhost`/`.local`/`10.`/`192.168.`/`172.16–31.` are allowed for local dev against self-hosted Convex), rejects scheme-less/malformed URLs. The FITNESS client previously accepted ANY scheme (the member-side `convex.ts` already required https). +5 jest cases (`secureBaseUrl.test.ts`); api-client suite 40/40; `npm run typecheck` clean. (2) `apps/mobile/app.json` — `android.usesCleartextTraffic:false` (Expo prebuild left it true in the release manifest → release AAB permitted HTTP).

**Note vs the 2026-07-10 requireCloudUrl consideration above:** that deferred item is about making the MEMBER client (`convex.ts`) MORE permissive (accept LAN http for physical-device local testing). This PR hardens the FITNESS client the OTHER direction (stricter). Different clients, different directions — `convex.ts` deliberately left untouched (still the operator's product/security call). If both land, the LAN allow-list in `assertSecureBaseUrl` is the pattern to reuse for `convex.ts`.

**Flagged, NOT fixed (out of scope for a transport PR):** prod Convex URLs in `app.json` (`convexProdBaseUrl`/`convexProdCloudUrl`) are unreferenced scheme-less placeholders → a production build still ships pointing at the DEV deployment (operator config + wiring); `packages/sync-queue` health aggregates persist as plaintext JSON in SQLite `genoly_sync.db` (at-rest-on-device encryption = larger follow-up).

**No live-Convex calls** — guard is pure, tests mock fetch (honors the no-live-Convex-test-runs rule). Merge queue: mobile #46 is independent of the web PRs; merge anytime.
## [2026-07-11] merge | Physical-device fixes (#47) — Health Connect visibility + fullscreen gestures; local APK v3

Operator on a real Samsung (local-Convex APK) hit two emulator-invisible bugs. #47:
1. "Health data unavailable" despite HC installed with data → Android 11+ package visibility: manifest lacked `<queries><package com.google.android.apps.healthdata></queries>` so getSdkStatus() couldn't see the provider. New `apps/mobile/plugins/withHealthConnectManifest.js` adds the queries block + android.permission.health.READ_{STEPS,ACTIVE_CALORIES_BURNED,DISTANCE} (rn-health-connect's own plugin only adds the rationale action). Verified in prebuilt manifest. Needs operator's HC device to fully confirm.
2. Fullscreen Explore no pan/pinch — RN <Modal> renders in its own view root the app-root GestureHandlerRootView doesn't cover; wrapped modal body in its own GestureHandlerRootView. Verifiable on emulator; textbook RNGH fix; inline gestures already worked so logic was sound.
tsc clean; 372/34. Independent of parallel PR #46 (transport hardening — enforces HTTPS on fitness client + android cleartext off; still OPEN, and its cleartext-off will conflict with local-http APKs, so local-APK builds must keep temp-overriding usesCleartextTraffic:true).
Rebuilt ~/Desktop/genoly-local-convex.apk (118 MB) on #47 + temp local-Convex config (LAN URLs + cleartext + convex.ts http-guard relax, all reverted). Connectivity re-verified via bad-cred probe → local Convex server response. NOTE: emulator has the release build (not dev-client). "Cloud²/Hyperion Payment System" is a separate operator app on the emulator.

## [2026-07-11] merge | Health Connect inverted constant + fullscreen close button (#48); local APK v4

Operator's Samsung, round 2. #47's manifest fix (queries+perms) was necessary but health STILL unavailable → real root cause: HealthConnectAdapter.isAvailable() compared getSdkStatus() `=== 1` with an inverted comment ("1=available"). rn-health-connect SdkAvailabilityStatus is 1=UNAVAILABLE, 2=PROVIDER_UPDATE, 3=AVAILABLE — so it returned false on EVERY device with Health Connect. Fixed to `=== 3` (SDK_AVAILABLE, inlined const). +HealthConnectAdapter.status.test.ts (3 tests pinning the mapping). Also #48: fullscreen close button was (a) jammed in the notch corner and (b) un-tappable because the ZoomPanView pan gesture swallowed the tap — moved to a box-none SafeAreaView overlay that's a SIBLING of the canvas (outside its gesture stack), inset + 48pt. tsc clean; 375 pass (+3)/34.

Rebuilt ~/Desktop/genoly-local-convex.apk (v4, 118 MB, all fixes) on temp local-Convex config (reverted). ON-DEVICE emulator verification: signup end-to-end WORKS against local Convex (real-email account created → correctly hit the Pro paywall — confirms local auth + Pro gate on-device). Could NOT reach fullscreen Explore to visually confirm the close-button tap: fresh gmail signup gets a non-e2e tenant slug (testData:setTenantPlanInternal is e2e-*-only), and the e2e admin's .test email is blocked by the release build's reserved-TLD client check. Close-button fix is structurally correct (gesture-swallow root cause + reposition); operator (already Pro on their Samsung) to confirm the visual. Emulator IME has a Samsung stylus popup that interferes with adb text entry — dismiss + retype. Health fix needs the operator's HC device to fully confirm (no HC provider on emulator).

## [2026-07-11] note | Prod-URL resolution — production build no longer points at dev (PR #49 OPEN)

Item 1 of the cross-repo security follow-ups (web brief `_security-followups-flagged-items-brief.md`; web items 2/3/4 = PRs #274/#275 + a decision memo). Branch `security/mobile-prod-url-resolution`, **PR #49 OPEN**.

**Finding (re-confirmed on disk):** app.json was static — `extra.convexBaseUrl`/`convexCloudUrl` held the DEV deployment for EVERY build, and `convexProdBaseUrl`/`convexProdCloudUrl` were scheme-less placeholders no code read. eas.json had no env switching, no app.config existed. So a production EAS build shipped pointing at dev — not a fallback, the only path.

**Fix:** new **`apps/mobile/app.config.ts`** (dynamic, extends app.json). `resolveConvexUrls()` selects by `process.env.EAS_BUILD_PROFILE`: **production** → operator EAS env vars `CONVEX_PROD_BASE_URL`/`CONVEX_PROD_CLOUD_URL`, validated https + `.convex.site`/`.convex.cloud`, and **throws at config-eval (build fails) if missing/placeholder/http** — injected not hardcoded (§3.10), can never silently fall back to dev; **dev/preview/local** → committed app.json dev URLs. Resolved values written back to `extra.convexBaseUrl`/`convexCloudUrl` so app code is unchanged. Dead placeholder keys removed from app.json. `AGENTS.md §8.1` corrected (it described a split that was never wired). Resolver is exported FROM app.config.ts (not a sibling module) because Expo's config loader can't resolve a nested `.ts` import at require time.

**Verification (LOCAL, no build/deploy):** `apps/mobile/app.config.test.ts` 9 jest cases (prod→prod URLs; prod missing/placeholder/http/wrong-host→throws; dev/preview/undefined→dev); real `npx expo config` across profiles (default→dev + placeholders stripped; `EAS_BUILD_PROFILE=production` no env → config eval FAILS exit 1; +valid env → prod URLs); full jest 384; `npm run typecheck` clean.

**Operator action:** set `CONVEX_PROD_BASE_URL` + `CONVEX_PROD_CLOUD_URL` as EAS env vars (production scope) before `eas build --profile production`. **Heads-up:** this branch was cut from clean HEAD; the uncommitted local-APK WIP on `main` (LAN URLs in app.json + a convex.ts edit, from the v4-APK work above) was preserved via stash and needs manual reconciliation with the removed placeholder keys when you next touch app.json.

## [2026-07-11] merge | Health Connect permission delegate (#50) — Grant-access crash; local APK v5

Layer 3 of the on-device Health Connect saga: after #47 (manifest visibility) + #48 (inverted SDK constant), Grant access hard-crashed natively ("Genoly keeps stopping"). Cause: rn-health-connect REQUIRES HealthConnectPermissionDelegate.setPermissionDelegate(this) in MainActivity.onCreate (README setup; requestPermission launches the contract via a lateinit launcher). Its own plugin doesn't add it; MainActivity.kt is generated → our withHealthConnectManifest plugin gained a withMainActivity mod injecting import + call (idempotent). Verified injected + Kotlin compiles (release build) + app launches clean with the delegate in onCreate. PR #50 merged (numbering: #49 taken by a parallel session's PR). APK v5 delivered to ~/Desktop/genoly-local-convex.apk (temp local config, reverted). Operator to confirm the real permission sheet on the Samsung.

## [2026-07-13] note | Rule #0 catch-up cascade — reconciled state after security + on-device fixes merged

Docs-only state cascade (no code) to bring this repo's memory-bank current; the wiki/current/* files were stale at "8 PRs open / #49 OPEN". Reconciled against actual merge history (`git log origin/main`, `gh pr list`), **not** the stale files. **main HEAD = `93181cd`.**

**Reconciled merge list — everything below is MERGED (nothing open):**
- The 2026-07-09 Phase-1/tree run — **#27–#40 all merged** 2026-07-09/07-10 (the stale "8 PRs open, NONE merged" claim was wrong). Fitness Steps 8/9/10/13 + four Pro-gated tree surfaces + follow-on device fixes (#36 minSdk 26, #37 icon/splash, #38 remove Pedigree/cap Fan at 3 gens/branding, #39 iOS build, #40 HealthKit new-arch).
- **#46** (`b0f569e`) — fitness client HTTPS transport guard (`assertSecureBaseUrl`, loopback/LAN exempt) + `android.usesCleartextTraffic:false`. **MERGED** (was "OPEN" in the [2026-07-11] transport note).
- **#49** (`633715b`) — `app.config.ts` resolves Convex URLs by `EAS_BUILD_PROFILE`; production reads operator EAS env vars and **fails closed** if unset (no more silent dev fallback). **MERGED** (was "OPEN" in the [2026-07-11] prod-URL note).
- **#50** (`33319ac`, commit msg mislabeled `#49`) — Health Connect permission delegate. **MERGED** (entry above).
- **#51** (`da20172`) — `ExploreCanvas.tsx` caps the SVG base raster (~36MB, uniform `fit` scale). Large tree at ±4 generations requested ~211MB → Android single-bitmap limit → native crash in `SvgView.onDraw`. viewBox stays content-space; pinch-zoom recovers detail. Verified on-device (release APK, inline + fullscreen ±4, no crash). **MERGED.**
- **#52** (`93181cd`) — added `expo-secure-store` + its config plugin. It was **never actually a dependency**: `SecureTokenStore` require()'d it in a try/catch and silently no-op'd on failure, so the fitness bearer token was never persisted → `getToken()` always null → "No bearer token available", health never synced on ANY build. `issue-token` itself always returned 200; the token just had nowhere to live. Hardened the store to `console.warn` in `__DEV__` if the module is missing. Verified end-to-end on a release APK vs local Convex: `issue-token → 200`, authenticated `sync/daily → 200`, Activity shows "Synced just now". **MERGED.**

**Verified this session:** `git log origin/main` HEAD `93181cd`; `gh pr list --state open` → none. The `~/Desktop/genoly-local-convex.apk` on disk is the rebuilt release APK carrying #51 + #52 (local-Convex config, reverted from the tree).

**Pending (carried forward — see `_work-summary-security-and-mobile-2026-07-13.md` §4):**
1. **Mobile EAS prod env vars** before any `eas build --profile production`: `CONVEX_PROD_BASE_URL` (`https://…convex.site`) + `CONVEX_PROD_CLOUD_URL` (`https://…convex.cloud`), production scope. Without them the build fails at config-eval (intended, #49). Dev/preview unaffected. (Web-side keys `MESSAGE_ENCRYPTION_KEY`/`MFA_SECRET_ENC_KEY` are the web repo's concern.)
2. **Local-APK WIP — reconcile, DO NOT blind-pop.** `git stash@{0}` ("operator local-APK WIP … pre-sync 2026-07-11") holds LAN-URL `app.json` + `convex.ts` edits cut before #49. Post-#49 per-build URLs come from `app.config.ts`, so the LAN hack must be reconciled with the new resolution, not restored. Current local-APK recipe = LAN URLs in app.json extra + expo-build-properties `usesCleartextTraffic:true` + `convex.ts` http-guard relax + `authSchemas.ts` reserved-TLD bypass (for `.test` e2e login), all reverted after build. `scratchpad/build-apk.sh` banks it.
3. **Device verification pending:** physical Samsung test of the delivered APK — grant Health Connect, confirm real step/calorie/distance sync (emulator has no HC provider so it shows the empty state). **iOS not rebuilt** — the `expo-secure-store` fix applies to iOS too but wasn't built/tested.
4. **Fitness token recovery (robustness follow-up):** per `mobile-sync-architecture.md §3`, design calls for re-minting on cold-start `/auth/me` 401 + a Settings "re-authenticate" action. Currently `issueToken` runs only at login/signup with a swallowed catch, so a one-time mint failure needs full sign-out/in to recover. Not blocking.
5. **Cross-repo (not mobile):** stale web PR #191 "polish: Phase 3b follow-up" — rebase-or-close decision needed.

## [2026-07-13] merge | Health read pipeline ROOT-CAUSED + FIXED (#53) — adapters honor OS grants; missing producer added; local APK v6

The "physical Samsung not grabbing health data" bug (dispatch `_mobile-health-sync-debug-brief.md`) is TWO independent structural bugs, both silent, both dating to the original Step 4/12/5 implementation (`51d5259`, 2026-05-29). H1 (stale LAN IP post-Mac-restore) was RULED OUT with evidence: the delivered APK baked `http://192.168.68.132:3210/:3211`, the new Mac kept the SAME LAN IP, local Convex up on 0.0.0.0, firewall off, `curl` 200 from the LAN address.

**Root cause 1 (both paths):** `HealthConnectAdapter.readDailyAggregates()` gated on a per-instance `initialized` flag set ONLY by `requestPermissions()` on that same object — but `createHealthAdapter()` is a fresh-instance factory, so every reader (challengeSync etc.) saw `false` and silently got `[]`, even right after a successful grant. Grants persist at the OS level; the flag never was the source of truth. Same bug in HealthKitAdapter (iOS) and even MockHealthAdapter (so the DEV mock toggle was dead too).

**Root cause 2 (fitness path):** the sync pipeline had drainers (`useDashboardData`, `backgroundSync`) but NO PRODUCER — `SyncQueue.enqueue()` had zero production call sites. `sync/daily` never received device data on any build. The earlier #52 "verified end-to-end" claim proved token+transport only: `drain()` short-circuits on an empty queue without any network call.

**Fix (PR #53, squash `d7c3eba`, merged):**
- health-sync adapters: lazy per-process client init on the read path; Android consults `getGrantedPermissions()` and reads only granted record types; new optional `getGrantedMetrics()` on the HealthAdapter interface (null on iOS — HealthKit hides read-auth state); Mock models persisted grants.
- NEW `apps/mobile/utils/healthSync.ts` producer: health store → `HealthEntryUpload` (computes `dateUtcStart`) → `queue.enqueue` with deterministic `${date}:${source}` ids. 30-day initial pull (§3.7) via new pref `genoly.lastHealthCollectAt`, 7-day steady state. Wired into dashboard refresh, background task, and permissions screen (day-one collect+drain after grant, `assumeEnabled` avoids the AsyncStorage race).
- Silent-failure hardening: `challengeSync` returns `reason` (`sync-disabled`/`store-unavailable`/`no-read-permission`/error message) + `__DEV__` warns; challenge detail toasts are actionable; login/signup `issueToken` failures warn + honest login toast; hub logs non-synced results in `__DEV__`.
- Tests 389→407 passed (34 skipped unchanged); `tsc --noEmit` clean. New suites: `HealthConnectAdapter.read.test.ts` (fresh-instance read regression), `healthSync.test.ts` (producer gating/window/mapping/ids).

**Local APK v6 delivered** to `~/Desktop/genoly-local-convex.apk` (118MB, main `d7c3eba` + temp local config, tree reverted clean). Same recipe as v5 + gradle needs `JAVA_HOME="/Applications/Android Studio.app/Contents/jbr/Contents/Home"` and `ANDROID_HOME=~/Library/Android/sdk` exported (fresh shells lack them; first run failed on both).

**Emulator verification (release APK, local Convex over LAN):** login as `test-admin@genoly.test` (temp authSchemas bypass) → Home with live server data; Settings shows account email (= authenticated fitness `GET /auth/me` with the persisted bearer token); Health Connect EXISTS on this Android-16 emulator image (older "no HC provider" note is stale) — permission flow exercised end-to-end incl. the new honest "No access granted" alert; after grant (HC ask-limit hit → granted the three `android.permission.health.READ_*` via `adb pm grant`, then Grant access succeeded) Health Sync shows **Enabled** and Activity shows **"Synced just now"** with the honest empty state (emulator HC store has no source apps → no records; expected). Zero crashes/JS errors in logcat. What the emulator CANNOT prove: real recorded steps flowing through (store is empty) — that's the operator's Samsung step.

**Operator handoff (Samsung, manual sideload — no USB):** install `~/Desktop/genoly-local-convex.apk` (same Wi-Fi as the Mac, local Convex up), sign in, Grant access on the permissions screen (real HC permission sheet), open Activity → real steps/calories/distance should appear and "Synced just now"; Challenges → a joined challenge → "Sync now" → step count toast. If anything fails it now says WHY (no-permission / disabled / store-unavailable toasts instead of silence).

## [2026-07-13] merge | HC grant recovery (#54) + full games port (#55); local APK v7

Two merges following the operator's Samsung field report on APK v6 ("No access granted", no permission sheet) plus a feature request (add all games to mobile).

**#54 (`3dea8d4`) — Health Connect grant recovery.** Root cause of the Samsung dead end: Health Connect RATE-LIMITS its permission contract (~2 denials — attempts were burned across the v3–v5 APK rounds incl. the pre-#50 native crash), after which requestPermission auto-resolves EMPTY with no UI, and the app had no way out. Fix: `requestPermissions()` pre-checks `getGrantedPermissions()` and short-circuits when the OS already holds grants (so granting manually in HC settings is honored WITHOUT relaunching the broken contract), requests only missing metrics, and trusts the post-contract OS grant state over the contract's return; new optional `openHealthSettings()` on the adapter interface (Android → HC settings screen; iOS best-effort Health app; Mock true); the permissions screen's failure alert gains an "Open Health Connect" action and keeps the user on-screen for the retry. +5 adapter tests. **Samsung recovery path: Grant access → alert → Open Health Connect → App permissions → Genoly → allow → back → Grant access again → succeeds.**

**#55 (`9a27a91`) — full games suite ported to mobile (operator request).** Home's Today's Pick was a "coming soon" toast. Now: `lib/gameRegistry.ts` rewritten to the web's flat multi-category model (the mobile mirror was still on the retired 2026-06-11 two-axis split — web moved on in W1 2026-07-06); shared pure libs `dailySeed`/`connectionsPuzzle`/`timelineTapGame` copied VERBATIM from web so daily puzzles hash identically cross-platform; `genolyApi` gains `getAtlasData`/`getTreeTimeline`/`getPersonPickerData` + `PersonDoc.archivedAt` + `RelationshipGraph.totalParentChildLinks`; games hub at `/games` (category sections, filter chips, `checkPlayable` lock states from `games.getGamesContext`); dynamic `/games/[gameKey]` route; Home gains a Games section (Today's Pick opens the real game). All 8 games native in `components/games/`: Wordle, Word Search, Atlas Quiz (d3-geo Equal Earth on react-native-svg, tap→`projection.invert`), Generation Sprint, Who Am I?, This or That, Family Connections, Timeline Tap. The two dailies fire `gameCompletions:recordDailyCompletion` once (idempotent, feeds contribution streaks) + social line via `getDailySocialStats`. Per-tree streaks/bests/daily snapshots in AsyncStorage under the web's EXACT localStorage keys. New deps: `d3-geo`, `topojson-client`, `world-atlas` (+types). RN adaptations: on-screen keyboards, tap-tap word-search selection, `Share.share` not clipboard, `AccessibilityInfo` reduce-motion, flash not CSS shake. Build note: the port was parallelized across 4 subagent sessions against a shared foundation (registry/libs/hub/route/Wordle pattern); all integrated on one branch, one squash merge.

**Verification:** typecheck clean; jest **419 passed / 34 skipped / 453** (new: gameRegistry suite ported from web; dashboard test moved to fake-timer date control for the new `pickTodaysGame()` signature). Emulator (release APK v7 vs local Convex over LAN): Home Games section renders; hub shows category chips + real lock states (Atlas Quiz correctly locked — e2e tree has no geocoded births); Family Wordle plays end-to-end (real 5-letter daily from tree persons, guess scored present/miss, keyboard state updates). HC grant-recovery flow not fully provable on emulator (needs the rate-limited state; pm-grant path verified in the v6 session).

**Local APK v7 delivered** to `~/Desktop/genoly-local-convex.apk` (118MB, main `9a27a91` + temp local config — LAN URLs/cleartext/convex.ts relax/authSchemas bypass — all reverted; tree clean). Jest/typecheck run BEFORE the temp edits.

**Operator handoff (Samsung):** install v7. (1) Health: Settings → Manage permissions → Grant access; if "No access granted" appears, tap "Open Health Connect" → App permissions → Genoly → allow Steps/Active calories/Distance → back → Grant access again → Activity should fill with real data (30-day initial pull). (2) Games: Home → Games section → play. Web-parity: same daily word/puzzle as genoly.org.

## [2026-07-13] merge | Android 14+ HC client declaration (#56) — the REAL grant blocker; local APK v8

Operator's second Samsung report on v7 nailed it: **Genoly never appears in Health Connect → App permissions / "available apps to connect"**, so grants were impossible even manually. True root cause (supersedes the v6 rate-limit theory as the primary blocker): on Android 14+ (HC in the OS) the system only recognizes an app as a Health Connect client when the manifest declares an exported `activity-alias` handling `android.intent.action.VIEW_PERMISSION_USAGE` + category `android.intent.category.HEALTH_PERMISSIONS` (guarded by `START_VIEW_PERMISSION_USAGE`). Neither our `withHealthConnectManifest` plugin nor rn-health-connect's added it — the library only injects the Android-13-era `ACTION_SHOW_PERMISSIONS_RATIONALE` filter. With the app unrecognized, `requestPermission()` always resolved empty with no UI — every layer above (#47 visibility, #48 constant, #50 delegate, #54 recovery UX) was necessary but this was the missing bottom layer. **#56 (`b06b34d`)**: plugin adds the alias (idempotent) + dedupes the library's re-appended rationale filters. Verified in the prebuilt AND compiled manifest (aapt) and via `pm dump` on the emulator (VIEW_PERMISSION_USAGE handler registered after install). Suite 419/34, tsc clean (plugin-only). Operator scope question answered in-session: the declaration registers a minimal 3-read-type HC integration for walking challenges; it does NOT rebrand Genoly as a health app (Play-side: declare the 3 read permissions in the Play Console health form at production time).

**Local APK v8 delivered** to `~/Desktop/genoly-local-convex.apk` (main `b06b34d` + temp local config, reverted; tree clean). Samsung flow: install v8 → Genoly should NOW appear under Health Connect → App permissions → allow Steps/Active calories/Distance (or just tap Grant access in Genoly — the real sheet should appear now) → Activity fills (30-day pull). Samsung Health was connected to HC by the operator this session, so the store has a live data source.

## [2026-07-13] merge | Health-data policy: 30-day history sync (#57) + user controls (#58) + web retention (#277); APK v9/v10

Operator data policy landed across both repos: **user-triggered 30-day import; 1-year server retention once collected; user can always remove or disconnect.**

- **#57 (`5e9bff7`)** — Settings → Health sync → **"Sync last 30 days"**: `collectHealthDataIntoQueue` gains a `windowDays` override (the first-ever collect fired when Health Connect held only today; source apps backfill HC on their own schedule, and the steady 7-day window would miss late-arriving days 8–30). Per-status result toasts. APK v9 delivered.
- **web #277 (`7ae9ae6`, deployed to local Convex)** — daily cron `purge-old-fitness-health-data` (365-day retention on `fitness_health_daily`, which was previously NEVER deleted anywhere) + bearer-authed `DELETE /api/fitness/health-data` + privacy-policy §8 retention statement + first fitness-backend test suite. See web log.md [2026-07-13].
- **#58 (`926a463`)** — the mobile controls: **"Turn off health sync"** (pref off + background task unregistered; OS grants untouched) and **"Delete my health data…"** (destructive confirm → server wipe via new api-client `deleteHealthData()` → `SyncQueue.clearAll()` (new; both stores) so stale local rows can't re-upload → `clearLastHealthCollectAt()` so a re-enable does a fresh 30-day pull). Suite 424/34, tsc clean.
...
## [2026-07-13] merge | Aggregate instant contract (#60) — corrects #59; APK v12

Operator field result on v11: Genoly OVERcounted (3,257 vs Samsung Health 2,735). Verified against rn-health-connect's native source (`HealthConnectUtils.kt`): BOTH time-filter variants run `Instant.parse()` on the JS strings — `getTimeRangeFilterLocal` (used by `aggregateGroupByPeriod`) parses the INSTANT then converts to device-local `LocalDateTime`. #59's naive local strings threw `DateTimeParseException` natively → the aggregate path never ran → the silent raw fallback's overlap double-counting overcounted. **#60 (`62b1fe5`)**: pass real local-midnight instants (native converts to local-day groups); bucket by the returned naive LocalDateTime group starts; regression tests pin the instant contract + bucketing. Suite 427/34, tsc clean. **APK v12 delivered** (main `62b1fe5` + temp local config, reverted; tree clean; emulator boot clean). Lesson banked: rn-health-connect timeRangeFilter strings must ALWAYS be instants; naive-local strings fail at the native boundary and our fallback masks it — read the library's Kotlin before trusting the TS types.

Calories/Distance "—" on the operator device = SOURCE-side: Samsung Health writes those record types to HC on its own schedule/for sessions; verify presence under Health Connect → Your health data. Genoly reads them the moment they exist.

## [2026-07-13] merge | On-device read diagnostics (#61); APK v13

Operator reported v12-era numbers unchanged (3,257) — but with sideload cycles we can't tell remotely whether the aggregate path ran, the fallback ran, or the old build was still installed. Guessing ends here: **#61 (`061bafa`)** adds a diagnostics layer. `HealthConnectAdapter.getReadDiagnostics()` records per-metric read outcome (path `aggregate`|`raw-fallback`, days with data, the aggregate error when the fallback engaged, and the window instants); the producer persists a snapshot (`genoly.lastHealthReadDiag`) stamped with `HEALTH_READ_REV` (`r61`); Settings → Health sync renders it (rev + time + day count + today steps + per-metric path/error). A single Settings screenshot now identifies the installed build AND the exact read behavior. Native conversions re-verified in lib source: `mapJsPeriodToPeriod` handles `{period:'DAYS',length:1}` cleanly; instant contract per #60. Suite 428/34; tsc clean. **APK v13 delivered** (main `061bafa` + temp local config, reverted; tree clean). Operator flow: install v13 → Activity Refresh → Settings screenshot of the "Last read (r61) …" line.

## [2026-07-13] merge | Aggregate tier chain (#62) — r61 diagnostics caught an UPSTREAM lib bug; APK v14

The r61 field screenshot delivered exactly what it was built for. Diagnostic line showed: `steps: aggregate ×1d` (steps ARE correct — Genoly now equals Health Connect's deduplicated total; the residual delta vs Samsung Health's dashboard is Samsung's export-vs-display behavior, confirmable in HC → Your health data → Steps) BUT `distanceMeters/caloriesActive: raw-fallback [agg err: Either use TimeRangeFilter with LocalDateTime or AggregateGroupByDurationRequest]`.

Root cause (verified in lib source): **react-native-health-connect builds the period-aggregate request inconsistently per record type** — ReactStepsRecord uses `getTimeRangeFilterLocal` (correct LocalDateTime contract) while ReactActiveCaloriesBurnedRecord/ReactDistanceRecord use `getTimeRangeFilter` (Instant), which the HC SDK rejects for period grouping. **#62 (`f4205e0`)**: per-metric three-tier chain — `aggregateGroupByPeriod` (calendar local days) → `aggregateGroupByDuration` (24h buckets anchored at local midnight; the Instant contract IS correct for the duration variant; ±1h only on DST-change days) → raw readRecords. Diagnostics record the serving tier; `HEALTH_READ_REV` → **r62**. Suite 428/34; tsc clean. **APK v14 delivered** (main `f4205e0` + temp local config, reverted; tree clean; r62 + duration tier verified in bundle).

Calories/Distance data availability remains SOURCE-side (HC held no such records for the operator's day — "(no data)" in the fallback too); with #62 they'll be read via the dedup path the moment Samsung Health writes them. Upstream lib fix candidate: PR to rn-health-connect making all record types use getTimeRangeFilterLocal for period requests — noted as a community follow-up.

## [2026-07-13] merge | r62 verified the tier fix; origin attribution added (#63); APK v15

Operator's r62 screenshot CONFIRMED #62 worked: `caloriesActive/distanceMeters: aggregate-duration` (tier 2 runs clean — the `[agg err]` shown is the retained period-tier note by design). Their "(no data)" is therefore a verified SOURCE-side absence: Health Connect holds no calorie/distance records; Samsung Health writes those types only with tracked sessions / on its own schedule. Steps remain = HC's deduplicated total (4,280 at 9:57 PM vs Samsung dashboard 3,300 — the delta is Samsung's export-vs-display behavior; HC → Your health data → Steps is the arbiter).

**#63 (`50f5819`)**: diagnostics now include `dataOrigins` from the HC aggregate results — the Settings line shows `src: shealth` (etc.) per metric, attributing exactly which app(s) wrote the displayed number. `HEALTH_READ_REV` → **r63**. Suite 428/34; tsc clean. **APK v15 delivered** (main `50f5819` + temp local config, reverted; tree clean).
