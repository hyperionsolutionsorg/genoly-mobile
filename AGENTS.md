...
The curated narrative is heavily forward-looking — most of the mobile codebase is still Expo boilerplate awaiting Phase 1 implementation. It describes BOTH what's here today AND the planned structure per `../genoly-family-web/docs/mobile-sync-architecture.md`.

**Tooling:** `graphifyy` is pipx-installed (binary `graphify`, v0.8.36+ as of 2026-06-10). Existing machines: `pipx upgrade graphifyy`. Fresh: `pipx install graphifyy` (upstream README recommends `uv tool install graphifyy`; pipx is fine). The optional `/graphify` Claude Code skill (`graphify claude install`) is available but not installed.

**Rules for AI tools (Claude Code, OpenCode/Kimi, Cursor, etc.):**

- **Before grepping, find, fd, or rg for code-structure questions, consult the graph first** — `graphify query "<question>"` against `graphify-out/graph.json`, or `docs/GRAPH_REPORT.md` for the curated narrative. It maps everything that exists today (Expo boilerplate + memory bank + planning docs) plus the planned packages (`api-client`, `health-sync`, `types`) with file pointers.
...
- The auto-installed `.claude/settings.json` and `.opencode/plugins/graphify.js` hooks check for `graphify-out/graph.json`. It exists — rebuilt 2026-06-10 with graphifyy 0.8.36 (**841 nodes, 967 edges, 67 communities**, AST-only, no key), so the `query`/`path`/`explain` graphify CLI commands ARE viable from a local shell (not from a Cowork sandbox).
