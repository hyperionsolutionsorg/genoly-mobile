...
There are TWO graph artifacts (mirrors `../genoly-family-web` §10.1):
- **`graphify-out/GRAPH_REPORT.md` + `graph.json` + `graph.html`** — structural, AST-driven, auto-generated by **Graphify Labs** (`graphifyy` on PyPI, binary `graphify`; graphifylabs.ai / safishamsi/graphify). No LLM key needed.
- **`docs/GRAPH_REPORT.md`** — the curated narrative, first written 2026-05-26 by Claude as the mobile-side Phase 3 deliverable. Companion to `../genoly-family-web/docs/GRAPH_REPORT.md` for the workspace.

The curated narrative was fully regenerated 2026-07-09 (Phase 1 complete: member app + fitness Steps 8/9/10/13 + the four Pro-gated tree surfaces). It describes what's here today plus the still-planned surfaces, per `../genoly-family-web/docs/mobile-sync-architecture.md`.

**Tooling:** `graphify` is installed at `~/.local/bin/graphify` (**v0.9.11** as of the 2026-07-09 new-Mac restore). Existing machines: `pipx upgrade graphifyy`. Fresh: `pipx install graphifyy` (upstream README recommends `uv tool install graphifyy`; pipx is fine). The optional `/graphify` Claude Code skill (`graphify claude install`) is available but not installed.

**Rules for AI tools (Claude Code, OpenCode/Kimi, Cursor, etc.):**

- **Before grepping, find, fd, or rg for code-structure questions, consult the graph first** — `graphify query "<question>"` against `graphify-out/graph.json` (natural-language question → scoped BFS subgraph with file:line pointers; far cheaper than a cold grep), or `docs/GRAPH_REPORT.md` for the curated narrative. It maps the real app today — the member side, fitness side, tree surfaces, packages — with file pointers.
- §3 (repo map) + §4 (current code) + §5 (packages) + §7 (decisions) + §8 (hard rules) should answer 90% of "where does X go?" without searching.
- **Don't grep `node_modules/`** — it's 100% noise. All real code lives in `apps/mobile/` + `packages/`.
- **`graphify-out/` is gitignored and did NOT survive the 2026-07-09 new-Mac restore — regenerated manually that day.** Current graph: **1738 nodes, 3322 edges, 103 communities** (graphify 0.9.11, AST-only, no key). ⚠️ There is **no auto-refresh git hook in this repo** (unlike genoly-family-web's `.husky/post-merge`); `.git/hooks` is empty. Run **`graphify update .`** yourself after structural changes to keep the graph current (a few seconds, no key). The `.claude/settings.json` + `.opencode/plugins/graphify.js` reminder hooks gate on `graphify-out/graph.json` existing — which it now does, so `query`/`path`/`explain` work from a local shell (not a Cowork sandbox). Restoring a tracked `.husky/post-merge` graphify hook to match the web repo is a recommended follow-up.
...
- `graphify affected "<symbol>"` — reverse traversal: what's impacted by a node
- `graphify update .` — re-extract AST after code changes (no LLM key needed; a few seconds)
- `graphify watch .` — keep `graphify-out/` live as you edit (recommended dev loop)
- `open graphify-out/graph.html` — force-directed network viewer (vis-network, dark theme, Leiden clusters)
- `graphify tree && open graphify-out/GRAPH_TREE.html` — optional hierarchical collapsible-tree view

**When the curated narrative goes stale:** when Phase 1 actually ships (token store + ApiClient skeleton + real screens replace boilerplate placeholders). The structural graph self-refreshes via `graphify update .`; the narrative is refreshed by a Claude session OR — if `GEMINI_API_KEY` / `ANTHROPIC_API_KEY` is set — `graphify extract . --backend <provider>` (adds semantic doc extraction).
