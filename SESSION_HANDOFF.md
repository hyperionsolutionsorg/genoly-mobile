...
| **Task #9 — GitHub Actions for build-android** | 2026-05-08 | **commit `03e5a73`**, pushed. `.github/workflows/build-android.yml` triggers on push to main (path filter on `apps/mobile/**` + `packages/**` + the workflow itself) and `workflow_dispatch`. Calls `eas build --platform android --profile preview --non-interactive --no-wait`. Auth via `EXPO_TOKEN` secret. Required adding `workflow` scope to existing GitHub PAT before push could go through. Verified end-to-end: workflow commit's own push auto-triggered EAS build `89183f18-3024-4785-8b97-d1d524303828`. **Phase 0 mobile is now fully closed.** |
| **Task #10 — EAS Build for Android** + **Task #11 — Phase 0 baseline verify** | 2026-05-08 | committed + pushed earlier today. EAS account `@hyperionsolutionsorg` (Hobby tier, free) created via GitHub OAuth. EAS project `@hyperionsolutionsorg/genoly-mobile` (ID `a059fba2-1ef7-431c-9870-6325603f3ada`) registered via `eas init`. `apps/mobile/eas.json` with three profiles: `development` (dev client + APK), `preview` (Genoly's primary distribution — APK, internal), `production` (AAB for future Play Store). Android keystore auto-generated and stored cloud-side. First preview APK built (`b0260446-e70b-4832-8ee6-567a5731545c`), installed on a real Android phone via QR-scan + Chrome sideload, all four tabs render with their icons, navigation works, no error overlay. Phase 0 baseline (Task #11) confirmed met: `tsc --noEmit` clean + working signed APK on device. |
...
| # | Task | Notes |
|---|---|---|
| — | **Phase 0 closed.** Phase 1 work begins next session. | Phase 1 splits across both repos: server-side mutation handlers in `genoly-family-web/convex/fitness/` (tasks #18, #19, #20 + the 20-endpoint `http.ts` registration), client-side `HealthAdapter` implementations in `packages/health-sync/`, `ApiClient` in `packages/api-client/`, login + settings + background sync wiring in `apps/mobile/`. Recommended starting point: server-side tasks first (so mobile has real endpoints to call), starting with `convex/http.ts` route scaffolding + the auth flow (`POST /api/fitness/auth/issue-token`). |
